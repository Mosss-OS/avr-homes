  ...
  return (
    <div className="group overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--shadow-card)] transition hover:-translate-y-0.5 hover:shadow-[var(--shadow-elevated)]">
      ...
      <div className="p-4">
        ...
        <Link to="/properties/$id" params={{ id: p.id }} className="mt-2 line-clamp-1 block text-sm font-medium hover:underline">
          {p.title}
        </Link>
        ...
        <div className="mt-3 flex items-center gap-4 border-t border-border pt-3 text-sm text-foreground/80">
          ...
          <span className="ml-auto inline-flex items-center gap-1.5"><Maximize2 className="h-4 w-4 text-primary" />{p.area.toLocaleString()} sqm</span>
        </div>
      </div>
    </div>
  );
