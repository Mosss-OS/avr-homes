...
class ReferralController
{
  ...
  /**
   * [Admin] List all referrals platform-wide.
   */
  public static function adminIndex(array $params): void
  {
    ...
    if ($status && in_array($status, ['pending','signed_up','upgraded','developer_referred','bulk_buyer_referred'])) {
      $where .= ' AND r.status = ?';
      $binds[] = $status;
    }
    if ($q) {
    ...
    $countStmt = $db->prepare("SELECT COUNT(*) FROM referrals r LEFT JOIN users u ON u.id = r.referrer_id LEFT JOIN users ru ON ru.id = r.referred_id WHERE 1=1 {$where}");
    ...
    $stmt = $db->prepare(
      "SELECT r.*, u.name as referrer_name, u.email as referrer_email,
              ru.name as referred_name, ru.email as referred_email
       FROM referrals r
  ...
  public static function adminStats(array $params): void
  {
    ...
    $stmt = $db->query(
      "SELECT
         ...
         SUM(CASE WHEN reward_paid = 0 THEN reward_amount ELSE 0 END) as pending_payout,
         SUM(reward_amount) as total_rewards
       FROM referrals"
    );
    $stats = $stmt->fetch();

    foreach (['total_referrals','total_referrers','signed_up','upgraded','developer_referred','bulk_buyer_referred'] as $k) {
      $stats[$k] = (int)($stats[$k] ?? 0);
    }
    foreach (['total_paid','pending_payout','total_rewards'] as $k) {
      $stats[$k] = (float)($stats[$k] ?? 0);
    }
  ...
  /**
   * [Admin] Update reward amount for a referral.
   */
  public static function adminUpdateReward(array $params): void
  {
    ...
    $amount = (float)($data['reward_amount'] ?? 0);

    $db = Database::getConnection();
    $stmt = $db->prepare('UPDATE referrals SET reward_amount = ? WHERE id = ?');
    $stmt->execute([$amount, $id]);

    Response::success([], 'Reward updated');
  }
  ...
  public static function adminMarkPaid(array $params): void
  {
    ...
    $db = Database::getConnection();
    $stmt = $db->prepare('UPDATE referrals SET reward_paid = 1, paid_at = NOW() WHERE id = ?');
    $stmt->execute([$id]);

    Response::success([], 'Reward marked as paid');
  }
